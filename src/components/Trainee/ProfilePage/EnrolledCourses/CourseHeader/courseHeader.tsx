...
const batchName = currentPath.split("/")[3]; // Adjusted for the correct segment
const decodedBatchName = decodeURIComponent(batchName);

// Capitalize the first letter of each word
const capitalizedBatchName = decodedBatchName
  .split(" ")
  .map((word) => word.charAt(0).toUpperCase() + word.slice(1))
  .join(" ");

console.log("Current batch name:", capitalizedBatchName); // Logs "Batch-1 React Js"


  // ✅ Fetch batch data using batchName instead of batchId
  const batch = useSelector((state: any) => state.batch.batchDataByName);

  console.log("📝 CourseHeader - Batch Data:", batch); // ✅ Debugging log

  if (!batch || !batch.course) {
    return (
      <div className="text-center text-gray-500">No batch data available.</div>
    );
  }
