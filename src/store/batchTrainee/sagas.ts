...
function* fetchBatchIdByTraineeIdSaga(action: ReturnType<typeof fetchBatchIdByTraineeIdRequest>): Generator<any, void, any> {
  try {
    // Make the API call to fetch batch IDs by trainee ID
...
export function* batchTraineeWatcherSaga() {
  yield takeLatest(types.FETCH_BATCHIDBYTRAINEEID_REQUEST, fetchBatchIdByTraineeIdSaga)
}
